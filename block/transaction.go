...
func (tx *Transaction) Deserialization(source *common.ZeroCopySource) error {
	...
	if length > base.TX_MAX_SIG_SIZE {
		return fmt.Errorf("transaction signature number %d execced %d", length, base.TX_MAX_SIG_SIZE)
	}
...
func (self *RawSig) Serialize(w io.Writer) error {
	err := common.WriteVarBytes(w, self.Invoke)
	if err != nil {
		return err
	}
	err = common.WriteVarBytes(w, self.Verify)
	if err != nil {
		return err
	}
...
func (self *RawSig) Deserialize(r io.Reader) error {
	invoke, err := common.ReadVarBytes(r)
	if err != nil {
		return err
	}
	verify, err := common.ReadVarBytes(r)
	if err != nil {
		return err
	}
	self.Invoke = invoke
...
func (self *Sig) GetRawSig() (*RawSig, error) {
	invocationScript := ProgramFromParams(self.SigData)
	var verificationScript []byte
	if len(self.PubKeys) == 0 {
		return nil, errors.New("no pubkeys in sig")
	} else if len(self.PubKeys) == 1 {
		verificationScript = ProgramFromPubKey(self.PubKeys[0])
	} else {
		script, err := ProgramFromMultiPubKey(self.PubKeys, int(self.M))
		if err != nil {
			return nil, err
		}
...
func (self *RawSig) GetSig() (Sig, error) {
	sigs, err := GetParamInfo(self.Invoke)
	if err != nil {
		return Sig{}, err
	}
	info, err := GetProgramInfo(self.Verify)
	if err != nil {
		return Sig{}, err
	}
...
func (self *Sig) Serialize(w io.Writer) error {
	...
	err := common.WriteVarBytes(w, invocationScript)
	if err != nil {
		return err
	}
	err = common.WriteVarBytes(w, verificationScript)
	if err != nil {
		return err
	}
...
func (self *Sig) Deserialize(r io.Reader) error {
	invocationScript, err := common.ReadVarBytes(r)
	if err != nil {
		return err
	}
	verificationScript, err := common.ReadVarBytes(r)
	if err != nil {
		return err
	}
	sigs, err := GetParamInfo(invocationScript)
	if err != nil {
		return err
	}
	info, err := GetProgramInfo(verificationScript)
	if err != nil {
		return err
	}
